...
const SkillsSection: React.FC = () => {
  ...
  const skills: Skill[] = [
    ...
    { name: 'CSS', icon: 'https://cdn.simpleicons.org/css3/1572B6', type: 'tech', color: '#1572B6' },
  ...
  const updateIndicatorPosition = () => {
    if (indicatorRef.current && techTabRef.current && toolsTabRef.current) {
      const activeTabElement = activeTab === 'tech' ? techTabRef.current : toolsTabRef.current;
      gsap.to(indicatorRef.current, {
        width: activeTabElement.offsetWidth,
        x: activeTab === 'tech' ? 0 : techTabRef.current.offsetWidth,
        duration: 0.3,
        ease: "power2.out",
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="flex justify-center mb-12 md:mb-16 animate-on-scroll">
          <div 
            ref={tabsRef}
          ...
          >
            <button
              ...
              className={`relative px-6 py-2 rounded-full text-sm font-medium z-10 transition-colors duration-200 ${
                activeTab === 'tech' ? 'text-white' : `${darkMode ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`
              }`}
              onClick={() => handleTabChange('tech')}
              onMouseEnter={() => {
                if (activeTab !== 'tech' && indicatorRef.current) {
                  gsap.to(indicatorRef.current, {
            ...
            <button
              ...
              type="button"
              className={`relative px-6 py-2 rounded-full text-sm font-medium z-10 transition-colors duration-200 ${
                activeTab === 'tools' ? 'text-white' : `${darkMode ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`
              }`}
              onClick={() => handleTabChange('tools')}
              onMouseEnter={() => {
                if (activeTab !== 'tools' && indicatorRef.current) {
                  gsap.to(indicatorRef.current, {
              ...
              .map((skill, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <motion.div
                      className="absolute inset-0 rounded-xl border-2 border-transparent group-hover:border-current"
